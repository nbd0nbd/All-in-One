...
# $allList = (Sort-Naturally (Get-ChildItem -Directory), (Get-ChildItem -File))


$folderList = (Sort-Naturally (Get-ChildItem -Directory)) |
Select-Object @{Label='Name';Expression={$_.BaseName}}, Extension, @{Label='Path';Expression={$_.FullName}} | Select-Object -ExpandProperty Name;
$fileList = (Sort-Naturally (Get-ChildItem -File)) |
Select-Object @{Label='Name';Expression={$_.BaseName}}, Extension, @{Label='Path';Expression={$_.FullName}} | Select-Object -ExpandProperty Name;

$folderList, $fileList | Set-Clipboard -Verbose;